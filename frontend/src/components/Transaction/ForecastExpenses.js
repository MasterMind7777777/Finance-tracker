...
const ForecastExpenses = () => {
  ...
  useEffect(() => {
    ...
    if (startForecast) { // only start the forecast when the button is clicked
      setLoading(true);
  ...
  return (
    <div>
      ...
      {loading
        ...
        : <div>Forecasted expenses: {forecast}</div> // render the forecast or nothing if it hasn't started
      }
    </div>
  );
};
