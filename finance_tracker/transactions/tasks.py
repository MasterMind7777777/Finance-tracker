...
# Set up logging
logger = logging.getLogger(__name__ + "_celery")
...
def forecast_expenses_task(self, user_id):
    try:
        user = User.objects.get(
            pk=user_id
        )  # get the user instance from user_id
    ...
    logger.info(
        "Days passed in the current month for user %s: %s",
        user_id,
        days_passed,
    )
    ...
    logger.info(
        "Forecasted expenses for user %s for the next month: %s",
        user_id,
        forecast_next_month,
    )
...
def compare_spending_task(user_id, friends_ids, category_id, period_days=30):
    ...
    user_transactions = Transaction.objects.filter(
        user=user, category=category, date__gte=start_date
    )
    ...
    user_avg_transaction = (
        user_total / user_num_transactions if user_transactions else 0
    )
    ...
    for request in accepted_friend_requests:
        ...
        friend = (
            request.to_user if request.from_user == user else request.from_user
        )
    ...
    if not friend_task_ids:
        ...
        for friend in friends:
            friend_data_task = process_friend_data_task.delay(
                friend.pk, category.name, start_date, period_days
            )
            # Store the task IDs
    ...
    for task_id in friend_task_ids:
        ...
        if not result.ready():
            logger.info("Task ID %s is not ready", task_id)
            return {"status": "Pending"}
        elif result.ready():
            output = result.result
            logger.info("Result for task ID %s: %s", task_id, output)
    ...
    all_friends_transactions = Transaction.objects.filter(
        user__in=friends, date__gte=start_date
    )
    ...
    for transaction in all_friends_transactions.iterator():
        if (
            task_results[transaction.user.pk]["result"][
                "best_matching_category"
            ]
    ...
    for friend_id, total in friends_spending.items():
        ...
        friends_avg_transactions[str(friend_id)] = (
            Decimal(total) / Decimal(count) if count > 0 else 0
        )
...
def process_friend_data_task(
    ...
    for task_result in calculate_category_results:
        category_scores[task_result.get("friend_category")] = task_result.get(
            "score"
        )
...
@shared_task(bind=True)
def calculate_category_similarity_task(
    self, friend_category_name, friend_category_id, category_name
):
...
def categorize_transaction_task(self, transaction_id, user_id):
    ...
    text = (
        ...
        + " "
        + uncategorized_transaction.description
    )
...
def find_matching_category_task(self, text, user_id, transaction_id):
    ...
    callback_subtask = calculate_similarity_task_chain.s(
        original_text_keywords_task["keywords"]
    ) | on_all_tasks_done.s(transaction_id)
...
def apply_recurring_transactions_task(frequency):
    ...
    latest_transaction_dates = (
        ...
        .values("recurring_transaction")
        .annotate(latest_date=Max("date__date"))
    )

    recurring_transactions = RecurringTransaction.objects.filter(
        pk__in=[
            item["recurring_transaction"] for item in latest_transaction_dates
        ]
    )
    ...
    for transaction_date in latest_transaction_dates:
        recurring_transaction = next(
            ...
            if rt.pk == transaction_date["recurring_transaction"]
        )
        latest_date = transaction_date["latest_date"]

        if (
            recurring_transaction.frequency == frequency
            and (current_date - latest_date).days >= 1
        ):
